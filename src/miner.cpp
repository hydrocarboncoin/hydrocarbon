...
{
    LogPrintf("NoorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Noorcoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running NoorcoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("NoorcoinMiner terminated\n");
        throw;
    }
}
