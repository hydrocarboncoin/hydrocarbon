...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Noorcoin"))
        return false;
...
{
    // Convert Noorcoin:// to Noorcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Noorcoin://"))
    {
        uri.replace(0, 12, "Noorcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Noorcoin.lnk";
}
...
{
    return GetAutostartDir() / "Noorcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Noorcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Noorcoin-qt [" + tr("command-line options") + "]                     " + "\n";
